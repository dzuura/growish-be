...
exports.register = async (req, res) => {
  ...
  if (userError) {
    ...
    return res.status(500).json({ error: `Failed to register in ${role} system`, details: userError.message });
  }

  const token = jwt.sign({ id: userData.id, role }, process.env.JWT_SECRET, { expiresIn: '1h' });
  res.status(201).json({ 
...
exports.login = async (req, res) => {
  ...
  if (user.role !== role) {
    return res.status(401).json({ error: 'Role mismatch. Please use the correct role for this account' });
  }
